# LANTERN - Cell Phenotyping

**LANTERN** (Lung Adenocarcinoma Neighborhood Tumor-immune Environment Recurrence Network) - Cell Phenotyping Module

The cell phenotyping framework is part of the LANTERN project for spatial analysis of lung adenocarcinoma tumor-immune microenvironments. This module assigns cell types to segmented nuclei based on marker expression patterns.
...
2. `input_cell_segmentation_folder`: the folder where the cell segmentation results are located at. This folder will have the same file organization (structure) as the input, given that the cell segmentation is done using our pipeline. For the cell segmentation pipeline, please refer to the `cell_segmentation` folder in this repository.
